...
# 项目根目录（本文件在 src/ 下，src 的上一级即项目根目录）
...
def init():
    ...
    # 加载路网图
    graph_path = ROOT / "data" / "ecnu_walk_merged.graphml"
    ...
    # 加载地点数据
    places_path = ROOT / "data" / "campus_places.json"
...
def _save_places():
    ...
    tmp = ROOT / "data" / "campus_places.json.tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(places, f, ensure_ascii=False, indent=2)
    os.replace(tmp, ROOT / "data" / "campus_places.json")
...
def serve_tile(z, x, y):
    """仅返回预下载的本地瓦片，无远程备用"""
    tile_dir = ROOT / "static" / "tiles" / str(z) / str(x)
...
def data_files(path):
    return send_from_directory(ROOT / "data", path)
